...
static void list_data_destruct(List_data *data) {
    animation_destruct(&data->animP);
    xfree(data);
}

static List_data *list_data(char *text, v4 color) {
    ...
    v2 dummy = {0};
    animation_construct(&result->animP, AnimationType_v2,
                        rect2_min_max((v2){.42f,0},(v2){.58f,1}),
                        &dummy, &dummy, &result->drawP);
