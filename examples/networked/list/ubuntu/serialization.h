...
static void
serialize_text(Stream *stream, char *text) {
    u8 length = (u8)strlen(text);
    serialize_u8(stream, length);
    memcpy(stream->storage + stream->at, text, sizeof(wchar_t)*length);
    stream->at += sizeof(wchar_t)*length;
}
...
deserialize_text(Stream *s, char text[256], u8 *length) {
    ...
    memcpy(text, s->storage + s->at, sizeof(wchar_t)*(*length));
    s->at += sizeof(wchar_t)*(*length);
}
