...
XWINMAIN()
{
    ...
    f32 seconds = 0.001f;
    state.timer = seconds;
    state.insert_sort_i = 0;
    state.insert_sort_j = narray(state.bar_sizes)-2;
...
void render_scene(void)
{
    ...
    for (s32 x=0; x<narray(state.bar_sizes); ++x)
    {
        ...
        if (x==state.insert_sort_i)
            bar_color = (v4f){.8f,.5f,.5f,1};
        //else if (x-1>state.insert_sort_j)
...
void insert_sort_iterate(void)
{
    if (sizes_greater(state.insert_sort_j-1, state.insert_sort_j))
        sizes_swap(state.insert_sort_j, state.insert_sort_j-1);
    
    if (state.insert_sort_j-- == 1)
    {
        state.insert_sort_j = state.insert_sort_i++;
    }
}
